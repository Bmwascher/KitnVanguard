...
1. **Install** the addon (CurseForge, Wago, or manual)
...
1. **ENCOUNTER_START** builds a healer list sorted by raid index
2. **UNIT_AURA** detects harmful dispellable auras in a 0.2s collection window
...
- **Text alert**: Center-screen display with class icon and configurable position/font/color
- **Sound alert**: LibSharedMedia-powered sound selection with channel control
- **Sync system**: AceComm broadcast on ready check / pull timer
- **Dwarf handling**: Dwarves/Dark Iron deprioritized (can Stoneform self-cleanse)
...
**Bitebtw** (GitHub: [Bmwascher](https://github.com/Bmwascher))
...
<sub>**Bundled libraries:** LibStub, CallbackHandler-1.0, LibGetFrame-1.0, LibCustomGlow-1.0, AceComm-3.0, AceSerializer-3.0, LibSerialize, LibDeflate, LibDataBroker-1.1, LibDBIcon-1.0, LibSharedMedia-3.0</sub>
